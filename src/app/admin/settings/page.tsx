...
export default function AdminSettingsPage() {
  ...
  return (
    <div>
      ...
      {/* Navigation Tabs */}
      <div className="flex gap-8 border-b border-gray-200 mb-8 font-mono">
        <button 
          onClick={() => setActiveTab("users")}
          className={`pb-4 text-sm font-bold uppercase tracking-widest transition-colors relative ${activeTab === "users" ? "text-[#C5A059]" : "text-gray-500 hover:text-gray-900"}`}
        >
        ...
        <button 
          ...
          className={`pb-4 text-sm font-bold uppercase tracking-widest transition-colors relative ${activeTab === "config" ? "text-[#C5A059]" : "text-gray-500 hover:text-gray-900"}`}
        >
                  ...
                  <tr key={u.id} className="hover:bg-gray-50/30 transition-colors group">
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-gray-50 border border-[#C5A059]/30 flex items-center justify-center font-mono font-bold text-[#C5A059] text-sm shrink-0 group-hover:border-[#C5A059] transition-colors">
                          {u.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="font-bold text-gray-900 text-sm mb-0.5">{u.name}</p>
                          <span className="text-[10px] font-mono text-gray-500">{u.id}</span>
                        </div>
                      </div>
                    </td>
                    <td className="p-5 font-mono text-gray-600">{u.email}</td>
                    <td className="p-5 font-mono">
                      {u.role === 'super_admin' ? (
                        ...
                        <select 
                          ...
                          className={`bg-gray-50 border text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-lg focus:outline-none cursor-pointer ${u.role === 'admin' ? 'border-[#C5A059] text-[#C5A059]' : u.role === 'manager' ? 'border-purple-500/40 text-purple-400' : u.role === 'vendor' ? 'border-blue-500/40 text-blue-400' : 'border-gray-600 text-gray-600'}`}
                        >
                    ...
                    <td className="p-5 font-mono text-xs text-gray-500 max-w-[200px] truncate">
                      {u.projects.join(", ")}
                    </td>
                    <td className="p-5 font-mono text-gray-500 text-[11px]">{u.lastLogin}</td>
      ...
      {activeTab === "config" && (
        <form onSubmit={handleSaveConfig} className="bg-white border border-gray-200 rounded-2xl p-8 shadow-xl max-w-4xl animate-in fade-in duration-500 space-y-8 font-mono text-xs">
          
          <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 flex flex-col gap-2 font-sans">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex flex-col gap-2">
              <label className="text-gray-500 uppercase tracking-widest text-[10px]">Gold Rate Ecosystem Markup</label>
              <input 
                type="text" 
                required
                value={goldMarkup}
                onChange={(e) => setGoldMarkup(e.target.value)}
                className="bg-gray-50 border border-gray-200 text-gray-900 rounded-xl px-4 py-3 focus:outline-none focus:border-[#C5A059] font-bold tracking-wider"
              />
              <span className="text-[10px] text-gray-500">Premium added to MCX live base rate.</span>
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-gray-500 uppercase tracking-widest text-[10px]">BVC Indemnity Insurance Premium</label>
              <input 
                type="text" 
                required
                value={bvcPremium}
            ...
            <div className="flex flex-col gap-2">
              <label className="text-gray-500 uppercase tracking-widest text-[10px]">Official Gold GST Slab</label>
              <input 
                type="text" 
                required
                value={goldGst}
            ...
            <div className="flex flex-col gap-2">
              <label className="text-gray-500 uppercase tracking-widest text-[10px]">Making Charges GST Slab</label>
              <input 
                type="text" 
                required
                value={makingGst}
            ...
            <div className="flex flex-col gap-2 md:col-span-2">
              <label className="text-gray-500 uppercase tracking-widest text-[10px]">Sequel Secure Logistics Base Transit Rate</label>
              <input 
                type="text" 
                required
      ...
      {showAddUserModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white border border-[#C5A059] rounded-2xl p-8 max-w-md w-full shadow-[0_0_50px_rgba(197,160,89,0.2)] relative">
            ...
            <p className="text-xs text-gray-500 mb-6">Mint a new authenticated user, vendor merchant, manager, or admin clearance.</p>
            ...
            <form onSubmit={handleAddUser} className="space-y-4 font-mono text-xs">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-gray-500 uppercase tracking-widest text-[10px]">Registered Google Email</label>
                <input 
                  type="email" 
                  required
              ...
              <div className="flex flex-col gap-1">
                <label className="text-gray-500 uppercase tracking-widest text-[10px]">Ecosystem Role Tag</label>
                <select 
                  value={newUserRole} 
              ...
              <div className="flex flex-col gap-1">
                <label className="text-gray-500 uppercase tracking-widest text-[10px]">Allocated Ecosystem Hubs</label>
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex flex-col gap-2 font-sans">
              ...
              <div className="bg-gray-50 p-3 rounded-xl border border-gray-200 text-[10px] text-gray-500 leading-relaxed mt-2 font-sans">
