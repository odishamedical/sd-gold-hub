            ...
            {liveRates ? (
              <div className="space-y-3">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-white font-bold tracking-wider">₹{liveRates.rate24K?.toLocaleString() || 'N/A'}/g</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-gray-400">22K Standard</span>
                  <span className="text-[#D4AF37] font-bold tracking-wider text-lg">₹{liveRates.rate22K?.toLocaleString() || 'N/A'}/g</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-gray-400">18K Rose/White</span>
                  <span className="text-white font-bold tracking-wider">₹{liveRates.rate18K?.toLocaleString() || Math.floor((liveRates.rate24K||0) * 0.75).toLocaleString()}/g</span>
                </div>
                <div className="text-[10px] text-center text-gray-500 mt-4 pt-2 border-t border-white/5 uppercase">
                  Updated: {liveRates.lastUpdated ? new Date(liveRates.lastUpdated).toLocaleString() : 'N/A'}
                </div>
              </div>
            ) : (
