        ...
        {shops.map(shop => (
          <div key={shop.id} className={`flex ${shops.length === 1 ? 'flex-col md:flex-row items-center md:items-start' : 'flex-col items-center text-center'} gap-4 md:gap-6`}>
            ...
            <div className={`flex-1 flex flex-col ${shops.length === 1 ? 'items-center md:items-start text-center md:text-left' : 'items-center text-center'} w-full`}>
              ...
              <Link href={`/gold-shop/${shop.slug}`} className={`mt-auto ${shops.length === 1 ? 'w-full md:w-auto' : 'w-full'} inline-flex items-center justify-center px-6 py-2 bg-[#C5A059] text-[#0A1021] text-xs font-bold uppercase tracking-wider rounded hover:bg-white transition-colors`}>
                Visit Showroom
              </Link>
            </div>
          </div>
