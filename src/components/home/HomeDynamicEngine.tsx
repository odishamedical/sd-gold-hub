                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {(() => {
                    ...
                    return finalJobs.map((job: any) => (
                      <Link href={`/jobs/${generateJobSlug(job)}`} key={job.id} className="rounded-2xl overflow-hidden group relative border border-[#D4AF37]/50 shadow-[0_10px_30px_rgba(212,175,55,0.15)] hover:shadow-[0_15px_40px_rgba(212,175,55,0.3)] lg:hover:-translate-y-1 transition-all duration-500 flex flex-col bg-gradient-to-br from-[#D4AF37] via-[#C5A059] to-[#8B6914] h-full">
                        <div className="bg-black/10 p-5 pb-4 border-b border-black/10 flex items-start gap-4 pointer-events-none">
                          {job.companyLogo ? (
