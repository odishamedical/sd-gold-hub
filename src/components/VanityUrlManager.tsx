...
import { createVanityRequest } from "@/lib/firestore/vanityRequests";
...
export default function VanityUrlManager({ 
  ...
  const handleCheckout = async () => {
    ...
    if (!shopId) {
      alert("Shop ID is missing. Please reload the dashboard.");
      return;
    }
  ...
  if (paymentSuccess) {
    return (
      <div className="bg-[#0B2B26] border border-[#C5A059] rounded-3xl p-8 sm:p-12 text-center shadow-2xl relative overflow-hidden animate-in fade-in zoom-in">
        ...
        <h2 className="text-3xl font-serif text-[#C5A059] font-bold mb-4">Request Submitted!</h2>
        <p className="text-gray-300 text-lg max-w-md mx-auto leading-relaxed">
          Your request for the custom Vanity URL has been submitted successfully. Our team will contact you shortly to complete the offline payment and verification.
        </p>
      </div>
    );
  }
        ...
        <div className={`transition-all duration-500 overflow-hidden ${hasSearched ? "max-h-[300px] opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="pt-6 border-t border-gray-700">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-6">
              ...
              >
                {isProcessingPayment ? "Processing..." : "Request Custom URL"}
              </button>
            </div>
            <p className="text-center text-xs text-gray-500 mt-6 font-medium">Your request will be submitted to our team. Payment will be collected offline.</p>
          </div>
        </div>
